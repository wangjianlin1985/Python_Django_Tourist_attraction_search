...
function initScenicTypeManageTool() {
	scenicType_manage_tool = {
		...
		search: function() {
			$("#scenicType_manage").datagrid("options").queryParams=queryParams; 
			$("#scenicType_manage").datagrid("load");
		},
		exportExcel: function() {
			$("#scenicTypeQueryForm").form({
			    url:"/ScenicType/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/ScenicType/update/" + rows[0].typeId,
					type : "get",
					data : {
